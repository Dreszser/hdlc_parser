...
hdlc_parser::Config SetupConfig(int argc, char** argv);
void PrintConfig(const hdlc_parser::Config& cfg);

int main(int argc, char** argv) {
    try {
        hdlc_parser::Config cfg = SetupConfig(argc, argv);
        PrintConfig(cfg);
...
hdlc_parser::Config SetupConfig(int argc, char** argv) {
    hdlc_parser::Config cfg = hdlc_parser::parse_args(argc, argv);
...
void PrintConfig(const hdlc_parser::Config& cfg) {
    ...
    std::cout << "output_sig: "
              << (cfg.output_file_sig.empty() ? "(none)" : cfg.output_file_sig)
              << "\n";
